...
<!--     <a style="position:fixed;bottom:39px;left:55%;height:70px;width:70px;" ><h1 onclick="up();" id="uploadpics" style="background-color:white;color:blue;border-radius:70px;padding-left:30px;padding-bottom:10px;"  >+</h1></a>
 -->    <a href="#www.google.com" ><button name="addfriends" >Add Friends</button></a>
    <a ><button name='friends'>Friends</button></a>
    <?php
    $reqs="select * from friends where request='$_SESSION[name]' and status='requested'";
    $t=$conn->query($reqs);
    if($t->num_rows>0){
       echo "<a ><button name='requests' style='background-color:rgb(82, 82, 236);color:white;' disabled >requests<span>*</span></button></a>";
    }
    else{
        echo "<a ><button name='requests' style='background-color:rgb(82, 82, 236);color:white;' disabled >requests</button></a>";
    }
    ?>
        ...
        <?php
            ...
            if($re->num_rows>0){
                while($rows=$re->fetch_assoc()){
                    $ss="select * from friends where user='$rows[name]'and request='$_SESSION[name]' and status='requested'";
                    ...
                    echo "<input type='submit' class='addfrndbtn' value='accept' id='$btn' onclick='add(`$rows[name]`)' ' >";
            ...
            if($y==0){
                echo "<div class='frienddiv' >";
                    ...
                    echo "<h1 style='margin-left:100px;' >no friends requests to you</h1>";
                    echo "<input type='submit' class='addfrndbtn' name='addfrnd1' value='Add friends' style='margin-left:250px;' >";
                    echo "</form>";
                    echo "</div>";
            }
        ?>
...
<script>
   function add(n){
        ...
        adf.addEventListener('submit',(event)=>{
            ...
            xhr.open('POST','acceptfrndreq.php',true);
