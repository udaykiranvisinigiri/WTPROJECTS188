...
<body>
    ...
    ?>
    <a href="#www.google.com" ><button style="background-color:rgb(82, 82, 236);color:white;" disabled >profile</button></a>
...
<div style="width: 82%;height: max-content;max-height:max-content;position: absolute;top: 5.5%;right: 30px;z-index:-1;" class="img" id="img" onclick="upr()" >
        <center><h1 style="color:blue;font-size:60px;" >YOUR POSTS</h1></center>
        <?php
            $re=$conn->query("select * from test where username='$_SESSION[name]';");
            if($re->num_rows>0){
                ...
                echo "</form>";
                echo "<form id='$com' class='comments' >";
                    echo "<input type='text' name='cfilename' value='$jpg' hidden >";
                    echo "<textarea type='text' name='comment' placeholder='type your comment..' style='margin-left:200px;padding-bottom:40px;' cols='40' ></textarea>";
                    echo "<input onclick='y(`$jpg`)' style='margin-left:10px;background-color:black;color:white;' type='submit' value='comment' >";
                    echo "</form>";
                    $cj=$jpg."s";
                    echo "<div id='$cj' ></div>";
                echo "</div>";
