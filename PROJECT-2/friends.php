...
<!--     <a style="position:fixed;bottom:39px;left:55%;height:70px;width:70px;" ><h1 onclick="up();" id="uploadpics" style="background-color:white;color:blue;border-radius:70px;padding-left:30px;padding-bottom:10px;"  >+</h1></a>
 -->    <a href="#www.google.com" ><button name='addfriends' >Add Friends</button></a>
    <a href="#www.google.com" ><button style="background-color:rgb(82, 82, 236);color:white;" disabled >Friends</button></a>
    <?php
        ...
        <?php
            ...
            if($re->num_rows>0){
                while($rows=$re->fetch_assoc()){
                    ...
                    $v=$rows['name']."vpf";
                    $g=$rows['name']."g";
                    echo "<input type='text' value='$rows[name]' id='$rows[name]' name='reqto' hidden >";
                        echo "<input type='button' class='addfrndbtn' value='unfriend' id='$btn'  >";
                        //echo "<input type='text' value='$rows[name]' id='$g' name='reqto' hidden >";
                        echo "<input type='submit' class='addfrndbtn' value='view profile' onclick='add(`$rows[name]`)' name='vbtn' style='margin-left:50px;' >";
                    }
...
<script>
    function vpf(v){
        let vv=document.getElementById(v);
        let n=new FormData(vv);
        let g=n.get('reqto');
        let gy=v+"g";
        let gn=document.getElementById(gy).value=g;
        
    }
   function add(n){
    /* sessionStorage.setItem('vpf','jk');
    location.href='/facebook/fprofile.php'; */
        let adf=document.getElementById(n);
        let btn=document.getElementById(`${n}btn`);
        adf.addEventListener('submit',(event)=>{
            event.preventDefault();
            let xhr=new XMLHttpRequest();
            xhr.open('POST','fpr.php',true);
